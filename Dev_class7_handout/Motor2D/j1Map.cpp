...
TileSet* j1Map::GetTilesetFromTileId(int id) const
{
	...
	while (tileset != nullptr)
	{
		...
		{
			if (id >= tileset->data->firstgid && id < tileset->next->data->firstgid)
			{
				return tileset->data;
			}
		}
		else return tileset->data;
...
bool j1Map::LoadProperties(pugi::xml_node& node, Properties& properties)
{
	...
	// an xml_node
	properties.draw = node.find_child_by_attribute("name", "Nodraw").attribute("value").as_bool();
	properties.navigation = node.find_child_by_attribute("name", "Navigation").attribute("value").as_bool();
